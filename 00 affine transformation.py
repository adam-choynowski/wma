...
def imshow(title, image):
    ...
    if k == ord("s"):  # Save image if "s" is pressed
...
translation_matrix = numpy.float32([
    [1, 0, 100],  # Translate by 100 pixels to the right
    [0, 1, 50]    # Translate by 50 pixels down
])
...
translation_matrix = numpy.float32([
    [1, 0, -100],  # Translate by 100 pixels to the left
    [0, 1, -50]    # Translate by 50 pixels up
])
img_translation = cv2.warpAffine(img, translation_matrix, (num_cols, num_rows))
